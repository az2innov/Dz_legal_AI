          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800">
                  ...
                  <ul className="space-y-3">
                      ...
                      {team.map(member => {
                          // Vérifie si ce membre spécifique est le propriétaire
                          const isMemberOwner = member.id === ownerId;
                          // Vérifie si c'est moi (connecté)
                          ...
                          return (
                            <li key={member.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-700">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] uppercase font-bold px-2 py-1 rounded-md border bg-primary-50 text-primary-700 border-primary-100">
                                        {member.role}
                                    </span>
                                    
                                    {/* CONDITION : Seul le propriétaire (connecté) peut voir le bouton supprimer */}
                                    {/* ET on ne peut pas se supprimer soi-même */}
              ...
              {/* FORMULAIRE INVITATION (Visible UNIQUEMENT pour le Propriétaire) */}
              {isCurrentUserOwner && (
                  <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 h-fit">
                      ...
                      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg mb-6">
                          <p className="text-sm text-blue-800 dark:text-blue-300 leading-relaxed">
                              Le collaborateur invité aura accès à tous les documents.
                          </p>
                      </div>
                      ...
                      <form onSubmit={handleInvite} className="space-y-4">
                          <div>
                              <label className="block text-sm font-medium text-gray-500 mb-1">
                                  {i18n.language === 'ar' ? 'البريد الإلكتروني' : 'Email du collaborateur'}
                              </label>
                              <div className="flex gap-2">
                                  <input 
                                    type="email" 
                                    className="flex-1 p-2.5 rounded-lg border outline-none dark:bg-gray-800 dark:text-white"
