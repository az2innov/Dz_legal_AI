...
const ChatPage = () => {
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-6rem)] -m-6 bg-white dark:bg-dark-bg relative">
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
              {sessions.map((session) => (
                <div
                  ...
                  className={`group flex items-center justify-between p-3 rounded-lg cursor-pointer text-sm transition-colors ${currentSessionId === session.id
            ...
            {messages.map((msg, idx) => {
              ...
              return (
                <div key={idx} className={`flex gap-4 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${isUser ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-primary-600'
                    }`}>
                  ...
                  <div className={`flex flex-col max-w-[85%] lg:max-w-[75%] ${isUser ? 'items-end' : 'items-start'}`}>
                    <div
                      className={`px-5 py-3.5 rounded-2xl text-sm leading-relaxed shadow-sm whitespace-pre-wrap ${isUser
          ...
          <div className="p-4 bg-white dark:bg-dark-bg border-t border-gray-100 dark:border-gray-800">
            <form onSubmit={handleSend} className="flex gap-3 relative max-w-4xl mx-auto">
              <input
                ...
                className={`flex-1 bg-gray-100 dark:bg-gray-800 border-none outline-none text-gray-900 dark:text-white rounded-xl py-3.5 px-5 focus:ring-2 focus:ring-primary-500/20 transition-all ${isTextArabic(input) ? 'text-right' : 'text-left'
                  }`}
